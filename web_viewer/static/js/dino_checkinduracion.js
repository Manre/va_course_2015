 ...
 */
	...
	function draw_data(array) {
	
	function draw_single_point(above_lim) {
		scatter.selectAll(".dot").checkin_dur(array.slice(0, above_lim)).enter().append("circle").attr("class", "dot")
				.attr("r", 6)
				.attr("cx", function(d) {
					return scale_x(d.X);
				})
	...
	var i = 0;
        d3.timer(function () {
            ...
            if (i >= array.length) {
                console.log("done");
                d3.select("#show_graphic_button").attr("disabled", null);
                return true;
            }
            return false;
        });
